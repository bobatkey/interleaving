...
request for input. As Kiselyov demonstrates, the fact that values of
type $\mathit{Reader}~m~a~b$ leave the source of the data that they
abstract is extremely powerful: different constructions allow readers
to be chained together, or connected to actual input/output devices,
while retaining the ability to perform concrete effects in the monad
$m$.  Kiselyov treats the $\mathit{Reader}~m~a~b$ type in
isolation. In this paper, we show that it can be regarded as an
instance of the general construction of the coproduct of a free monad
and an arbitrary monad, building on the general foundation of initial
$f$-and-$m$-algebras.
...
% effects. As we shall see in the rest of this paper, the abstraction of
% $f$-and-$m$-algebras provides us with a convenient interface for
...
%FIXME: reference ``two-level types''.
