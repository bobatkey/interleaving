...
\begin{abstract}
  ...
  interleaved with computational effects. For example, lists can be
  interleaved with input/output actions. We demonstrate that the
  abstraction of initial $f$-and-$m$-algebras, originally due to
...
like $\mathit{foldr}$ rather than through direct use of recursion are
twofold. Firstly, we are automatically guaranteed several desirable
properties, such as totality (on finite input), without having to do
any further reasoning. Secondly, functions defined using
$\mathit{foldr}$ obey a uniqueness property that allows us to easily
derive further properties about such functions. The style of
...
built upon the category theoretic foundation of initial
...
reasoning on values of recursive datatypes that goes far beyond lists
and $\mathit{foldr}$.

In this paper, we present a class of recursive datatypes where direct
use of the initial $f$-algebra methodology does not provide the right
level of abstraction. We consider recursive datatypes that interleave
pure data with effectful computation. For example, lists of characters
that are interleaved with the effectful input computations that read
them from some external source can be described by the following
...
properties (we demonstrate this in \autoref{sec:direct-eappend}). By
abstracting out the effectful parts, we arrive at the concept of
initial $f$-and-$m$-algebras, where a functor $f$ describes the data,
and a monad $m$ describes the effects. Initial $f$-and-$m$-algebras
represent a better level of abstraction to deal with interleaved data
and effects. The key idea is to separate the concerns of dealing with
pure data, via $f$-algebras, and effects, via
$m$-Eilenberg-Moore-algebras. We shall see (in
\autoref{sec:f-and-m-append}) that this separation has the following
benefits:
\begin{itemize}
\item \emph{Definitions} of functions on datatypes that interleave
  ...
  data. We will use the example of adapting the append function on
  lists to the case of lists interleaved with effects. The pure part
  of the computation remains the same, while the effectful part is
  straightforward.
\item \emph{Proofs} about functions on interleaved datatypes also
  ...
  demonstrate this though the proof of associativity for list append,
  which carries over almost unchanged from the pure case, except for
  an additional side condition that is discharged almost trivially.
\end{itemize}
...
We now motivate the consideration of interleaved data and effects by
looking at a function in the Haskell standard library that is not as
...
this function postpones the actual input effects until the list is
...
$\mathit{hGetContents}$, with the following undesirable consequences:
\begin{itemize}
\item Input/output errors that occur during reading (e.g.~network
  ...
  scope in which the exception may be thrown.
...
generalises the definition of $\mathit{List_{io}}$ that we gave in the
introduction:
\begin{displaymath}
  \begin{array}{ll}
  ...
  \end{array}
\end{displaymath}
A value of type $\mathit{List}~m~a$ consists of an effect described
by $m$, then either a $\mathsf{Nil_m}$ to indicate the end of the
list, or a $\mathsf{Cons_m}$ with a value of type $a$ and more
list. Thus this type describes lists of values of type $a$ interleaved
with effects from the monad $m$.

We can re-implement $\mathit{hGetContents}$ to generate values of type
$\mathit{List}~\mathit{IO}~\mathit{Char}$, which makes the
interleaving of effects and $\mathit{IO}$ actions explicit. A
(inefficient due to lack of buffering) implementation can be given in
terms of the standard Haskell primitives for performing IO on file
handles:
\begin{displaymath}
  \begin{array}{l}
...
the monad $m$, yielding either a result $\mathsf{Yield}~b$, or a
request for input. As Kiselyov shows, it is possible to define a
$\mathit{Monad}$ instance for $\mathit{Reader}~m~a$, which allows for
convenient composition of stream readers that may perform effects.

The $\mathit{List}~m~a$ and $\mathit{Reader}~m~a~b$ datatypes follow
similar patterns of interleaving pure data with computational
effects. The abstraction of $f$-and-$m$-algebras provides us with a
convenient interface for programming with and reasoning about
datatypes with interleaved effects.
...
definitions. The basic structure of the data is as follows:
\begin{itemize}
\item In \autoref{sec:f-algebras}, we recall the standard definitions
  ...
  $f$-algebras (\proofprinref{pp:initial-alg}), and demonstrate the
  initial $f$-algebra abstraction can be thought of as an abstract
  interface for programming and reasoning against.
\item We initiate our main running example of list append and its
  associativity property in \autoref{sec:pure-append}. In this
  section, we make use of the initial $f$-algebra methodology to
  define list append, and also to show how
  \proofprinref{pp:initial-alg} is used to prove the associativity
  property.
\item In \autoref{sec:monads}, we recall the definition of monads, and
  briefly discuss the work of Fokkinga and Pardo on recursion schemes
  in the presence of effects. As we note there, their work covers the
  case of effectful computations on pure data, whereas we wish to
  consider computations on effectful data.
...
\item In \autoref{sec:f-and-m-algebras}, we present the definition of
  ...
  the definition of list append on lists interleaved with effects, and
  its associativity property. This time, we are able to reuse the much
  of the definition and proof from the pure case in
  \autoref{sec:pure-append}, and the additional work that we need to
  carry to deal with effects is minimal.
\item Finally, in \autoref{sec:impl-f-and-m}, we show that the
  construction of initial $f$-and-$m$-algebras can be reduced to
  initial $(f \circ m)$-algebras. Consequently, we are able to give a
  generic construction of initial $f$-and-$m$-algebras for arbirary
  functors $f$ and monads $m$.
\end{itemize}
...
We now recall the concept of (initial) $f$-algebras, and describe the
accompanying definition and proof principles. We will make use of the
basic definitions of the polymorphic identity function $\mathit{id} =
\lambda x.~x$ and function composition $g \circ h = \lambda
x.~g~(h~x)$.

\subsection{Basic definitions}

The initial $f$-algebra methodology uses functors $f$ to describe the
individual ``layers'' of recursive datatypes. Formally, functors are
defined as follows:
\begin{definition}\label{defn:functor}
  A functor is a pair $(f, \mathit{fmap}_f)$ of a type operator $f$
  and a function $\mathit{fmap}_f$ of type:
  \begin{displaymath}
...
the carriers of initial $f$-algebras. The uniqueness requirement
...
We now give two examples of initial $f$-algebras. The first example
shows that standard recursively defined Haskell datatypes can be
retrofitted with the initial $f$-algebra structure. The second example
shows that it is possible, in Haskell, to construct an initial
$f$-algebra for any functor $(f,\mathit{fmap_f})$.

\begin{enumerate}
\item The functor $\mathit{ListF}~a$ describes the individual layers
  of a list:
  \begin{displaymath}
    ...
    \end{array}
  \end{displaymath}
  The following definitions witness the standard Haskell list datatype
  ...
  \begin{displaymath}
    ...
    \end{array}
  \end{displaymath}
\item We can define the carrier of an initial $f$-algebra for an
  arbitrary functor $(f, \mathit{fmap}_f)$ as a recursive datatype:
  \begin{displaymath}
...
initial $f$-algebras are isomorphic, using the initial algebra
property to define the translations, and \proofprinref{pp:initial-alg}
to prove that they are mutually inverse.
...
use $f$-and-$m$-algebras to simplify the definition and proof, with
the benefit that we will be able to reuse much of the definition and
proof that we give in this section.
...
case of lists interleaved with effects.
...
The obvious approach to defining functions on datatypes such as
$\mathit{List}~m~a$ and $\mathit{Reader}~m~a~b$ is to look directly to
the initial $f$-algebra methodology. By inspecting the types
...
Equipped with this observation, we can adapt the definition of
$\mathit{append}$ that we gave in \autoref{sec:pure-append} to the
setting of lists interleaved with effects. As above, we program and
reason against the abstract interface of initial algebras. We assume
that an initial $(\mathit{ListF}~a \circ m)$-algebra
$(\mu(\mathit{ListF}~a \circ m), \mathit{construct})$ exists, and we
write $\fold{-}$ for the unique homomorphism induced by initiality. We
now define $\mathit{eAppend}$:
\begin{displaymath}
  \begin{array}{l}
