...
\begin{abstract}
  ...
  this article.  \pattynote{Replace ``by the authors of the present
    article'' with something that gives Bart and Neil credit for their
    contributions to the FoSSaCS version.}  In this article we aim to
  introduce the general concept of initial $f$-and-$m$-algebras to a
...
of the current article \cite{atkey12fibrational}.  \pattynote{Replace
  ``by the authors of the present article'' with something that gives
  Bart and Neil credit for their contributions to the FoSSaCS
  version.}  In this article, we aim to introduce the concept of
initial $f$-and-$m$-algebras to a general functional programming
audience and show how they can be used to structure and reason about
functional programs in practice, without the heavy category-theoretic
prerequisites of that previous work.
...
\paragraph{Non-termination} A second scenario in which implicitly
interleaved effects arise is built-in to every Haskell type: the
possibility of non-termination while inspecting a pure value. Haskell
dictates a lazy evaluation strategy, in which the computation of a
value is only invoked if the value is actually used. For the purposes
of reasoning about Haskell programs, we can model the possibility of
non-termination using the \emph{lifting} or \emph{non-termination}
monad, $(-)_\bot$. This monad adds a bottom element $\bot$ to a type,
representing the possibility of non-termination at that type, and
every Haskell type is implicitly lifted using this monad.
...
infinite value is evaluated on demand as the structure is explored. In
fact, we implicitly used this facility in our definition of
...
in \autoref{sec:f-and-m-alg-impl}.)  \pattynote{Actually, we've used a
  subscript on $\mathit{MuFM_0}~f~m$, too...} This definition makes
it clear that the datatypes we are considering interleave pure data,
...
\item In \autoref{sec:impl-f-and-m}, we show that the construction of
  ...
  $f$ and monads $m$.
\item \todo{FIX THIS} In
  \autoref{sec:coproducts-with-free-monads}, we present an extended
  example of the use of initial $f$-and-$m$-algebras. We reconstruct a
  result of Hyland, Plotkin and Power on the construction of
  coproducts of free monads with arbitrary monads, using
  $f$-and-$m$-algebras. This construction has practical interest, in
  that it provides a general construction of monads that interleave
  abstract and concrete commands, as seen in the
  $\mathit{Reader}~m~a~b$ monad in the previous section. The monad
...
\begin{proofprinciple}[Initial $f$-Algebras]\label{pp:initial-alg}
  ...
  \end{displaymath}
\end{proofprinciple}
\pattynote{PP1 can be stated as a bi-implication.}
...
$\mathit{Mu}~(\mathit{ListF}~a)$ are not equal \pattynote{Replace
  ``equal'' with ``identical''? Surely they are equal in the
  categorical sense of both being initial...}, but they are both
...
\pattynote{Same comment as above.}
...
\begin{definition}
  ...
  \begin{align}
    ...
    l \circ \mathit{return}_m & = \mathit{id} \\
    \label{eq:em-alg-join}
    l \circ \mathit{join}_m & = l \circ \mathit{fmap}_m~l
  \end{align}
\end{definition}
...
\pattynote{Do we want more categorical diagrams?}
...
\begin{definition}
  ...
  \end{displaymath}
\end{definition}
\pattynote{Number the last two equations for reference below, say as
  (*) and (**).}
...
\begin{proofprinciple}[Initial $f$-and-$m$-Algebras]
  ...
  function $g :: \mu(f|m) \to a$, we can prove the equation:
  \begin{displaymath}
    \eFold{k}{l} = g
  \end{displaymath}
  ...
  \end{equation}
\end{proofprinciple}
\pattynote{PP2 can be stated as a bi-implication.}
...
\begin{example}
  ...
  \begin{displaymath}
    ...
    \end{array}
  \end{displaymath}
\pattynote{Point out that, if not for the List constructor, this would
  be join.}
  ...
  $\mathit{List}~m~a$:
\pattynote{Is the type of $\eFold{-}{-}$ correct? Should it be 
{\tt (ListF a b -> b) -> (m b -> b) -> List m a -> b?}}
  \begin{displaymath}
    \begin{array}{l}
      \eFold{-}{-} :: (f~a \to a) \to (m~a \to a) \to \mathit{List}~m~a \to a \\
      \eFold{k}{l} = \mathit{loop} \\
      \quad\begin{array}{r@{\hspace{0.4em}}l@{\hspace{0.5em}}c@{\hspace{0.5em}}l}
        \kw{where} & \mathit{loop}~(\mathsf{List}~x) &=& l~(\mathit{fmap}_m~\mathit{loop'}~x) \\
  ...
  \end{displaymath}
\end{example}
\pattynote{Include the type of loop above.}
...
two properties of $\mathit{eAppend}$ from the fact that it 
\pattynote{Actually, $\lambda xs. eAppend\, xs\, ys$!}
is constructed as a $(\mathit{ListF}~a)$-algebra homomorphism. We have
one property for each of the constructors of the type constructor
$\mathit{ListF}~a$:
\begin{align}
...
\end{align}
\pattynote{The first is by (*), the second is from (**), which is the
  same as (21).}

Again by construction, we also know that $\mathit{eAppend}$
\pattynote{Actually, $\lambda xs. eAppend\, xs\, ys$!}  is an
...
\end{equation}
\pattynote{Also by (**) or (21). Here, we should point out that the
  return/bind style interpretation of (26): $in_m$ is essentially
  join, so the lefthand side of (26) is essentially bind. Using
  ``let'' syntax {\tt let x = e1 in e2} for {\tt bind (\ x -> e2) e1},
  this means that (26) is {\tt eAppend (let xs = x in return xs) ys =
    let xs = x in eAppend xs ys}. This is a familiar commuting
  conversion saying that the first thing eAppend does is evaluate its
  first argument.}
...
\pattynote{So $\lambda x. eAppend\, x\, y$ is an E-M-algebra homomorphism,
  but $eAppend \, x$ isn't.}
 If we let the monad $m$ be the $\mathit{ErrorM}$
...
In the current authors' previous work \cite{atkey12fibrational}
\pattynote{Same comment as elsewhere.}
, this
...
way.  In essence, iteratees are descriptions of functions that
alternate reading from some input with effects in some monad,
eventually yielding some output. Kiselyov captured this using the
following datatype, which follows the same pattern of mutual recursion
as the $\mathit{List}~m~a$ datatype declaration:
\begin{displaymath}
  \begin{array}{ll}
...
\begin{displaymath}
  ...
  \end{array}
\end{displaymath}
Consequently, we could use \proofprinref{pp:initial-f-m-alg} to reason
about programs involving iteratees. For example, we could prove that
$\mathit{Reader}~m~a~b$ is a monad whenever $m$ is. However, we can
see iteratees as an instance of a yet more general construction:
\emph{the coproduct of a free monad with an arbitrary monad}. Monad
coproducts provide a general and canonical way of specifying the
combination of two monads \cite{luth02composing}. Almost trivially,
once we observe that $\mathit{Reader}~m~a~b$ is the coproduct of two
monads, we can immediately deduce that it is a monad, rather than
having to prove this fact as a special case for this particular
type. As we will see below, in \autoref{sec:construct-coproducts}, the
coproduct of a free monad and an arbitrary monad can be
straightforwardly constructed using initial $f$-and-$m$-algebras.
...
Although Pardo~\cite{pardo04combining} worked in the same effectful
setting as Fokkinga, he transferred even more origami programming
ideas to the effectful setting: in addition to defining the
aforementioned monadic $\mathit{fold}s$ (catamorphisms), he dualised
them to give monadic $\mathit{unfold}$s (anamorphisms) for structuring
corecursive programs with monadic effects, and also defined monadic
hylomorphisms to support even more general ways of structuring monadic
computations and combining their results. Interestingly, monadic
hylomorphisms do achieve some interleaving of recursive calls to
effectful computations with other computations, but the computations
they structure must still consume pure data. Finally, Pardo developed
rules for fusing monadic programs structured using the monadic
constructs he defined.
...
The work of Filinski and St\o{}vring is undoubtedly the most closely
related to ours. As we do in this article, they give $\mathit{fold}s$
for datatypes with proper interleaving of effects. They do so first
considering the case of lazy datatypes, and then generalising to
datatypes that interleave monadic effects other than nontermination
with pure data. To first model the way laziness interleaves the
possibility of nontermination at any point in the production of a data
structure, and then to model more general interleavings of effects,
Filinski and St\o{}vring work in the specific category CPO of
$\omega$-cpos and (total) continuous functions. They also work with a
specific grammar of what might be called ``effectful regular
...
$\mathit{fold}$s from Definition~\ref{Defn11}.  Such monadic
...
induction, it is perhaps surprising that the papers
preceding~\cite{filinski07inductive} did not derive induction rules or
other proof principles for effectful datatypes. However, Filinski and
St\o{}vring give a principle of {\em proof by rigid induction} for
such datatypes that is a variant of those of both Lehmann and
Smyth~\cite{ls81}, and of Crole and Pitts~\cite{cp92}. It supports the
...
$f$-and-$m$-algebras to support more generally.  \pattynote{We don't
  explicitly give proof principles, so it's hard to draw the same
  parallel for them as for definitional principles.} The results
reported here thus extend both the definitional principles of Filinksi
and St\o{}vring for structuring recursion over effectful datatypes,
and their proof principles for reasoning about computations over such
datatypes, to the general category-theoretic setting and to arbitrary
functors.  Filinski and St\o{}vring also give fusion rules for
effectful streams (although not for arbitrary effectful datatypes),
and illustrate the extension of relational reasoning to effectful
datatypes. We consider neither fusion rules nor relational reasoning
in this article.

Overall, then, we see that this article generalises previous
extensions of the initial algebra methodology to the effectful setting
in three ways: it handles arbitrary functors, rather than special
classes of functors; it handles actual interleaving of effects and
data, rather than just the wrapping of pure results in effectful
contexts; and it gives proof principles for reasoning about
interleaved effectful computations, rather than just constructs for
structuring those computations. \pattynote{We don't actually identify
  the proof principles we illustrate, so be careful here.}
...
\begin{proof*}
  ...
  \end{displaymath}
\pattynote{One reviewer missed the connection here with eAppend. Not
  sure why, but clarify?}  Close inspection of $h$ reveals that it has
the same structure as the definition of $\mathit{eAppend}$ in terms of
initial $f$-algebras we gave at the start of
\autoref{sec:direct-eappend}. Therefore, as we noted in the
introduction to this subsection, the construction we are building here
abstracts out the common parts of proofs and definitions on effectful
datatypes.
