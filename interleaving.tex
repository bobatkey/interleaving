...
Haskell's lazy datatypes can thought of as pure data interleaved with
...
algebra that describes the pure part of the datatype, and $m$ is a
...
data and effects. The key idea is to separate the concerns of dealing
with pure data, via $f$-algebras, and effects, via
...
$f$-algebras can also be made explicit.
...
lazy evaluation strategy where the computation of a value is only
...
non-termination by using the \emph{lifting} or \emph{non-termination}
monad, $(-)_\bot$, which adds a bottom element $\bot$ to a type,
representing the possibility of non-termination. Every Haskell type is
implicitly lifted using this monad.
...
data structures. Laziness means that the computations that generate an
infinite value are evaluated \emph{on demand} as the structure is
...
assumption that they are only applied to finite lists (for example,
the standard $\mathit{reverse}$ function). Likewise, when reasoning
about Haskell programs it often is assumed that lists are finite, so
that standard techniques like induction may be applied.
...
$\mathsf{Cons}_{\mathit{lazy}}$ constructor with a value of type $a$
and another $\mathit{List_{lazy}}~a$. (Note that, for simplicity's
...
domain-theoretic techniques (Pitts, \cite{pitts-relational}, provides
...
$f$-and-$m$-algebras \cite{filinski07inductive}.
