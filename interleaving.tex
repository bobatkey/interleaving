...
$m$-Eilenberg-Moore-algebras, they can be though of as a special kind
of $f$-algebra that interacts well with computational effects
described by a monad $m$. We will introduce
...
$f$-structure of values to a value\todo{Change suggested by
  R2}. Formally, $f$-algebras are defined as follows:
\begin{definition}
...
homomorphism (\emph{i.e.}, a stucture preserving map\todo{Suggested by
  R2}) between them:
\begin{definition}
...
\begin{definition}
  An \emph{initial $f$-algebra} is an $f$-algebra
  $(\mu \! f, \mathit{in})$ such that for any $f$-algebra $(a, k)$,
  there exists a unique $f$-algebra homomorphism from
  $(\mu \! f, \mathit{in})$ to $(a, k)$; we write this homomorphism as
  $\fold{k}$, which is a function of type $\mu f \to a$.\todo{suggested by R2}
\end{definition}
...
\todo{paragraph inserted to answer R2's reorganisation suggestion}We
demonstrate these complications through an extension of the list
...
we will discover that $f$-and-$m$-algebras yields a more satisfactory
solution.
...
fact that the initial $f$-algebra abstraction is unaware of the
presence of effects.
...
\todo{Added paragraph for R3}This proof, and the $f$-algebra proof in
\autoref{sec:direct-eappend}, are both generic in the monad $m$ that
we use to represent effects. In particular, if we instantiate $m$ to
be the non-termination monad, then we have proved that list append for
Haskell's standard lazy lists is associative, without having to
explicitly deal with a Cpo semantics.
...
effects other than just nontermination.
